...
def train(model, train_loader, eval_loader, args, qid2type):
    ...
    output = args.output
    optim = torch.optim.Adamax(filter(lambda p: p.requires_grad, model.parameters()), lr=0.0009)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (v, q, a, b, hintscore, type_mask, notype_mask, q_mask) in tqdm(enumerate(train_loader), ncols=100,
            ...
            if mode == 'new':
                if epoch == 0:
                    model.debias_loss_fn1 = Plain().cuda()
                    model.debias_loss_fn2 = LearnedMixin(args.entropy_penalty).cuda()

                # pred, loss, _ = model(v, q, a, b, None, 1 - (epoch / num_epochs) * (epoch / num_epochs), None)
                pred, loss, _ = model(v, q, a, b, None, 1 - (epoch / num_epochs)**(1), None)
                # pred, loss, _ = model(v, q, a, b, None, 0.5, None)
                # print("testbvjdbvjdvd")
                if (loss != loss).any():
...
def evaluate(model, dataloader, qid2type,epoch):
    ...
    with open(os.path.join('jsons', name), 'w') as f:
        for v, q, a, b, qids, _, q_mask in tqdm(dataloader, ncols=100, total=len(dataloader), desc="eval"):
            ...
            pred, _, _ = model(v, q, None, None, None, 0, None, loss_type=None)
            ...
            for j in range(len(qids)):
                ...
                ans_tmp_dic={}
                ans_tmp_dic['image_id']=int(qid//1000)
                ans_tmp_dic['question_id']=int(qid)
                if batch_score[j]==1:
                    ans_tmp_dic['answer'] = True
                else:
                    ans_tmp_dic['answer'] = False

                # ans_tmp_dic['pre']=pred
                # print(type(pred))
                # ans_tmp_dic['ground']=a
                # print(type(a))
                json.dump(ans_tmp_dic,f)
    ...
    #
    #         # ans_tmp_dic={}
    #         # ans_tmp_dic['image_id']=qid//1000
    #         # ans_tmp_dic['question_id']=qid
    #         # ans_tmp_dic['pre']=pred
    #         # ans_tmp_dic['ground']=a
    #
    #         # pic_ans.append(ans_tmp_dic)
    #
    #
    #         if typ == 'yes/no':
    #             score_yesno += batch_score[j]
