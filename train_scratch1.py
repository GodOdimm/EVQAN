...
def train(model, train_loader, eval_loader, args, qid2type):
    ...
    output = args.output
    optim = torch.optim.Adamax(filter(lambda p: p.requires_grad, model.parameters()), lr=0.0006)
    ...
    model.train(True)
    # print(results["score"], results['score_yesno'], results['score_other'], results['score_number'])
    ...
    for epoch in range(num_epochs):
        ...
        for i, (v, q, a, b, hintscore, type_mask, notype_mask, q_mask) in tqdm(enumerate(train_loader), ncols=100,
            ...
            #########################################
            assert mode in ['updn', 'inv_sup', 'v_debias', 'q_v_debias', 'v_inverse',
                            'new'], " %s not in modes. Please \'import train_GGE as train\' in main.py" % mode
            if mode == 'new':
                if epoch == 0:
                    for p1, p2 in model.named_parameters():
                        if not (p1.startswith('classifier') or p1.startswith('debias_loss_fn') or p1.startswith('v_net') or p1.startswith('q_net') or p1.startswith('v_att')):
                        # if not (p1.startswith('classifier') or p1.startswith('debias_loss_fn') or p1.startswith('v_net') or p1.startswith('q_net')):
                        # if not (p1.startswith('classifier') or p1.startswith('debias_loss_fn') or p1.startswith('v_net')):
                        # if not (p1.startswith('classifier') or p1.startswith('debias_loss_fn') or p1.startswith('q_net')):
                        # if not (p1.startswith('classifier') or p1.startswith('debias_loss_fn')):
                            p2.requires_grad = False
                    model.v_att = NewAttention(2048,1024,1024).cuda()
                    model.q_net = FCNet([1024, 1024]).cuda()
                    model.v_net = FCNet([2048, 1024]).cuda()
                    model.classifier =  SimpleClassifier(
                        args.num_hid, args.num_hid * 2, 2274, 0.5).cuda()
                    model.debias_loss_fn = LearnedMixin(args.entropy_penalty).cuda()
                    # model.debias_loss_fn = Plain().cuda()
                    # model.debias_loss_fn = Focal().cuda()
                    optim = torch.optim.Adamax(filter(lambda p: p.requires_grad, model.parameters()), lr=0.0006)
                ...
                ## first train
                pred, loss, _ = model(v, q, a, b, None, q_mask)
                ...
                pred, loss, _ = model(v, q, a2, b, None, q_mask)
                ...
                if topv == -1:
                    ...
                    v_grad_mask = (v_grad_sum >= 0.35).long()
                    v_grad_mask[:, 0] = 1
                ...
                pred, _, _ = model(v, q, None, b, v_mask, q_mask)
                ...
                pred, loss, _ = model(v, q, a2, b, v_mask, q_mask)
                ...
                v_mask = torch.zeros(v.shape[0], 36).cuda()
                visual_grad_cam = visual_grad.sum(2)
                hint_sort, hint_ind = hintscore.sort(1, descending=False)

                v_ind = hint_ind[:, :top_hint]
                v_grad = visual_grad_cam.gather(1, v_ind)

                if topv == -1:
                    v_grad_score, v_grad_ind = v_grad.sort(1, descending=False)
                    v_grad_score = nn.functional.softmax(v_grad_score * 10, dim=1)
                    v_grad_sum = torch.cumsum(v_grad_score, dim=1)
                    v_grad_mask = (v_grad_sum <= 0.35).long()
                    v_grad_mask[:, 0] = 1
                ...
                if random_num <= qvp:
                    ## first train
                    pred, loss, word_emb = model(v, q, a, b, None, None)
        ...
        model_path = os.path.join(output, 'model-final.pth')
        torch.save(model.state_dict(), model_path)
...
def evaluate(model, dataloader, qid2type):
    ...
    for v, q, a, b, qids, _, q_mask in tqdm(dataloader, ncols=100, total=len(dataloader), desc="eval"):
        ...
        # hintscore = Variable(hintscore, requires_grad=False).cuda()
        pred, _, _ = model(v, q, None, None, None, None, loss_type=None)
