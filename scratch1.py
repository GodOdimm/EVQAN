...
def parse_args():
    ...
    parser.add_argument(
        '--mode', default="base",
        choices=["updn", "base", "v_inverse","v_debias", "inv_sup", "gge_iter", "gge_tog", "gge_d_bias", "gge_q_bias", "new"],
...
def main():
    ...
    if dataset == 'cpv2':
        ...
        qid2type = {}
        for now in tmp:
            qid2type[str(now['question_id'])] = now['answer_type']
    else:
        with open('util/qid2type_%s.json' % args.dataset, 'r') as f:
            qid2type = json.load(f)

    ckpt = torch.load('/content/drive/MyDrive/vqa-cp/logs/[1]/model-9.pth')
    # states_ = ckpt
    ...
    print(train_dset.entries[0])

    train_loader = DataLoader(train_dset, batch_size, shuffle=True, num_workers=0)
    # train_loader = DataLoader(train_dset, batch_size, shuffle=False,
    #                           sampler=ClassAwareSampler(train_dset.entries), num_workers=0)
